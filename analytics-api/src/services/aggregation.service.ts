...
export class AggregationService {
  ...
  async aggregateDailyMetrics(date: Date): Promise<void> {
    ...
    const quizEvents = events.filter(e => 
      ...
      e.eventData && 
      (e.eventData as any).score !== undefined
    );
    
    const avgQuizScore = quizEvents.length > 0
      ? quizEvents.reduce((sum, e) => sum + ((e.eventData as any).score || 0), 0) / quizEvents.length
      : 0;
    ...
    const activeUserIds = [...new Set(events.filter(e => e.userId).map(e => e.userId!))] as string[];
   ...
   */
  async calculateUserMetricsSnapshot(userId: string, date: Date = new Date()): Promise<void> {
    const startOfDay = new Date(date);
    startOfDay.setHours(0, 0, 0, 0);
    ...
    // Quiz performance
    const quizEvents = userEvents.filter(e => 
      e.eventType === 'quiz_completed' && 
      e.eventData && 
      (e.eventData as any).score !== undefined
    );
    ...
    const totalCorrect = quizEvents.reduce((sum, e) => sum + ((e.eventData as any).correct || 0), 0);
    const totalQuestions = quizEvents.reduce((sum, e) => sum + ((e.eventData as any).total || 0), 0);
    const avgScore = quizzesCompleted > 0
      ? quizEvents.reduce((sum, e) => sum + ((e.eventData as any).score || 0), 0) / quizzesCompleted
      : 0;
   ...
   */
  private calculateStreak(events: { timestamp: Date }[]): { currentStreak: number; longestStreak: number } {
    if (events.length === 0) {
   ...
   */
  async aggregateDateRange(startDate: Date, endDate: Date): Promise<void> {
