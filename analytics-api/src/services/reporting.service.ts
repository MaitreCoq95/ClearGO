...
export class ReportingService {
   ...
   * Get report history
   */
  async getReportHistory(userId?: string, limit = 50): Promise<any[]> {
   ...
   */
  private addSummarySheet(sheet: ExcelJS.Worksheet, data: any, periodStart: Date, periodEnd: Date) {
    // Title
   ...
   */
  private addComplianceSheet(sheet: ExcelJS.Worksheet, data: any) {
    sheet.columns = [
   ...
   * Add progress sheet
   */
  private addProgressSheet(sheet: ExcelJS.Worksheet, data: any) {
    sheet.columns = [
      { header: 'Utilisateur', key: 'user', width: 25 },
   ...
   */
  private addPerformanceSheet(sheet: ExcelJS.Worksheet, data: any) {
    sheet.columns = [
      { header: 'Utilisateur', key: 'user', width: 25 },
  ...
  async generateCsvReport(
    ...
    if (type === 'progress') {
      ...
      (data.performance || []).forEach((r: any) => {
...
<body>
   ...
   */
  async fetchReportData(type: string, periodStart: Date, periodEnd: Date): Promise<any> {
  ...
  async cleanupOldReports(): Promise<number> {
    ...
    // Find old reports
    const oldReports = await prisma.generatedReport.findMany({
      where: {
