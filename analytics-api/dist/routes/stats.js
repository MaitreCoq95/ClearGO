...
async function statsRoutes(fastify) {
    ...
    fastify.get('/stats/summary', async (request, reply) => {
        ...
        // Today's events
        const todayEvents = await index_1.prisma.event.count({
            where: { timestamp: { gte: today } },
        });
        // This week's events
        const weekEvents = await index_1.prisma.event.count({
            where: { timestamp: { gte: weekAgo } },
        });
        // This week's unique users
        const weekUsers = await index_1.prisma.event.groupBy({
            by: ['userId'],
            where: {
        ...
        const totalUsers = await index_1.prisma.userProgress.count();
        // Top event types today
        const topEvents = await index_1.prisma.event.groupBy({
            by: ['eventType'],
            where: { timestamp: { gte: today } },
    ...
    fastify.get('/stats/realtime', async (request, reply) => {
        ...
        const recentEvents = await index_1.prisma.event.groupBy({
            by: ['eventType'],
            where: { timestamp: { gte: hourAgo } },
            _count: { eventType: true },
        });
        const activeUsers = await index_1.prisma.event.groupBy({
            by: ['sessionId'],
            where: { timestamp: { gte: hourAgo } },
        });
        return {
